...
# Load sample list
sample_file = f"{WORK_DIR}/list/nichols.sampleList.txt"
...
rule all:
    ...
    """
    input:
        expand(f"{ASSEMBLY_DIR}/{{sample}}/references/CP004010.2.{{sample}}.masked.fa", sample=samples),
...
rule mask_reference:
    ...
    input:
        ...
        bed = f"{WORK_DIR}/bed/nichols/{{sample}}.bed",
        reference = REFERENCE
    output:
        f"{ASSEMBLY_DIR}/{{sample}}/references/CP004010.2.{{sample}}.masked.fa"
...
rule split_bed:
    group: "all_rules_group"
    input:
        f"{WORK_DIR}/bed/nichols/{{sample}}.bed"
    output:
    ...
    shell:
        ...
        while IFS=$'\\t' read -r chrom start end tag; do
            ...
            echo -e "$chrom\\t$start\\t$end\\t$tag" >> "/work/users/f/a/farhang/scratch/2024-08-01_TPal_ONT/assembly_results/nichols/{wildcards.sample}/references/tags_masked/$gene.bed"
        done < {input}
        """
