...
# Load sample list
SampleList = pd.read_csv(f"{WORK_DIR}/list/ss14.sampleList.txt", header=None)
...
rule extract_200nc:
    ...
    input:
        fastq = f"{WORK_DIR}/raw_long_read/ss14/{{sample}}.fastq"
    output:
...
rule alignment_200nc:
    ...
    input:
        r1 = f"{ASSEMBLY_DIR}/{{sample}}/tmp_short_fastq/{{sample}}.R1.fastq",
        r2 = f"{ASSEMBLY_DIR}/{{sample}}/tmp_short_fastq/{{sample}}.R2.fastq",
        masked_reference = f"{ASSEMBLY_DIR}/{{sample}}/references/CP004011.1.{{sample}}.masked.fa"
    output:
...
rule filter_long_reads:
    group: "read_process_6"
    input:
        long_reads = f"{WORK_DIR}/raw_long_read/ss14/{{sample}}.fastq",
        read_names = f"{ASSEMBLY_DIR}/{{sample}}/long_read/{{sample}}.{{gene}}.read_names"
    output:
