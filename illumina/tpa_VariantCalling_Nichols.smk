...
rule fq_seqtk_sub:
	...
	input:
		fq_1 = "processed_fastq/nichols/host_filtered/{sample}_R1.host_filtered.fq.gz",
		fq_2 = "processed_fastq/nichols/host_filtered/{sample}_R2.host_filtered.fq.gz",
		fq_clean = "processed_fastq/nichols/host_filtered/{sample}.host_filtered.fastq.gz"
	output:
...
rule mark_dups:
	...
	output:
		...
		logfile = "bam/nichols/aln/{sample}.ss14.log.txt"
	resources: time_min=120, mem_mb=8000
	threads: 4
	log: "bam/nichols/aln/{sample}.ss14.log.txt"
	shell:
		r"""
		set -euo pipefail
